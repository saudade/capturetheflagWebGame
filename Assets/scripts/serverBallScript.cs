	...
	void OnCollisionEnter2D(Collision2D coll)
	{
		...
		if (coll.gameObject.tag == "Player" && teamColor!=coll.gameObject.GetComponent<serverBallScript>().teamColor)
		{
			...
			if(hasRollingBomb==true)
			{
				...
				//if flag carrriers kiss, they should not respawn in jail
				if(hasFlag==true && coll.gameObject.GetComponent<serverBallScript>().hasFlag==true)
				{
					ballJail=false;
					coll.gameObject.GetComponent<serverBallScript>().flagKiss = true;
				}
				else if(flagKiss==true)
				{
					ballJail=false;
				}
